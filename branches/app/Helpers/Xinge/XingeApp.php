...
class XingeApp {
	...
	public static $accessId = ''; //应用的接入Id
	public static $secretKey = ''; //应用的skey

	public function __construct()
	{
		self::$accessId = config('xinge.accessId');
		self::$secretKey = config('xinge.secretKey');
	}
	...
	public function PushAccountIos($content, $account, $environment)
	{
		$mess = new MessageIOS();
		$mess->setAlert($content);
		$ret = $this->PushSingleAccount(0, $account, $mess, $environment);
		return $ret;
	}
	...
	public function PushAllAndroid($title, $content)
	{
		...
		$action->setActionType(ClickAction::TYPE_ACTIVITY);
		$mess->setAction($action);
		$ret = $this->PushAllDevices(0, $mess);
		return $ret;
	}
	...
	public function PushAllIos($content, $environment)
	{
		$mess = new MessageIOS();
		$mess->setAlert($content);
		$ret = $this->PushAllDevices(0, $mess, $environment);
		return $ret;
	}
	...
	public function PushTagAndroid($title, $content, $tag)
	{
		...
		$action->setActionType(ClickAction::TYPE_ACTIVITY);
		$mess->setAction($action);
		$ret = $this->PushTags(0, array(0=>$tag), 'OR', $mess);
		return $ret;
	}
	...
	public function PushTagIos($content, $tag, $environment)
	{
		$mess = new MessageIOS();
		$mess->setAlert($content);
		$ret = $this->PushTags(0, array(0=>$tag), 'OR', $mess, $environment);
		return $ret;
	}
	...
	protected function callRestful($url, $params)
	{
		...
		$sign = $paramsBase->generateSign(RequestBase::METHOD_POST, $url, self::$secretKey);
		$params['sign'] = $sign;
	...
	{
		if((self::$accessId) >= XingeApp::IOS_MIN_ID and $message instanceof MessageIOS)
			return true;
		else if((self::$accessId) < XingeApp::IOS_MIN_ID and $message instanceof Message)
			return true;
		else
			return false;
	}
